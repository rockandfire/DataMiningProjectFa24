...
def extract_text_perms(card):
	...
	# 
	rule_phrases = []
	rule_components = rules.strip().replace(',', '').replace('.', '').replace(':', '').split(' ')
...
def get_similarity(card_a, card_b):
	return len(set(card_a).intersection(set(card_b))) / (len(card_a) * len(card_b))
...
def apriori(commander_name, clusters, cards):
	...
	while cluster_pos < len(clusters) and len(current_deck) < 63:
		...
		for card in current_deck[:(len(current_deck) // 50) + 1]:
			...
			for index in clusters[cluster_pos].index:
				...
				card_similarity = get_similarity(cards.loc[card], cards.loc[index])
	...
	# print(cards.loc[max_index_a], cards.loc[max_index_b])
	# print(len(current_deck))
	# print(current_deck)
	# print(similarities)
	# print(cards[cards.index.isin(current_deck)][['name', 'edhrecRank']])
	rules = []
...
for i in cards.index:
	rule_phrases.append(extract_text_perms(cards.loc[i]))

# Perform K-Means clustering for k clusters on cards
clusters = k_means(10, cards)
...
rules = apriori(commander_name, clusters, cards)
